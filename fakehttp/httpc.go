...
func (cl *Client) getURL() string {
	url := cl.Host + cl.Url
	//Vlogln(2, "Заголовки когда берем токен:", req.Header)
...
func (cl *Client) getToken() (string, error) {
	...
	req.Header.Set("Host", cl.CDom)
	req.Header.Set("User-Agent", cl.UserAgent)
	//Устанавливаем хост отличный от таргета
	req.Host = cl.CDom
...
func (cl *Client) getTx(token string) (net.Conn, []byte, error) { //io.WriteCloser
	...
	txbuf := bufio.NewReaderSize(tx, 1024000)
...
func (cl *Client) getRx(token string) (net.Conn, []byte, error) { //io.ReadCloser
	...
	rxbuf := bufio.NewReaderSize(rx, 1024000)
